...
const productSelect=document.querySelector('#inquiry-product');
...
function render(){
 ...
 document.querySelector('.collection-count').textContent=filter==='saved'?`Your wishlist · ${visible.length} ${visible.length===1?'gift':'gifts'}`:`${visible.length} little wonders`;
...
function notify(message){const t=document.querySelector('#toast');t.textContent=message;t.classList.add('visible');clearTimeout(window.toastTimer);window.toastTimer=setTimeout(()=>t.classList.remove('visible'),2400)}
...
function updateDialogSave(){if(activeProduct){const b=document.querySelector('#dialog-save');b.innerHTML=icon('heart')+(saved.includes(activeProduct.id)?'Saved to wishlist · Remove':'Save to wishlist');b.setAttribute('aria-pressed',String(saved.includes(activeProduct.id)))}}
function openProduct(id,trigger){activeProduct=products.find(p=>p.id===id);if(!activeProduct)return;lastTrigger=trigger;document.querySelector('#dialog-title').textContent=activeProduct.name;document.querySelector('#dialog-description').textContent=activeProduct.description;setImage(activeProduct.images[0]);document.querySelector('#thumbnails').innerHTML=activeProduct.images.length>1?activeProduct.images.map((img,i)=>`<button class="${i===0?'active':''}" data-image="${img}" aria-label="View ${activeProduct.name} image ${i+1}" aria-pressed="${i===0}"><img alt="" src="${productImage(img)}"></button>`).join(''):'';updateDialogSave();dialog.showModal();document.body.style.overflow='hidden'}
function setImage(img){const el=document.querySelector('#dialog-image');el.src=productImage(img);el.alt=activeProduct.name;document.querySelectorAll('[data-image]').forEach(b=>{const active=b.dataset.image===img;b.classList.toggle('active',active);b.setAttribute('aria-pressed',String(active))})}
document.addEventListener('click',e=>{const product=e.target.closest('[data-product]'),save=e.target.closest('[data-save]'),f=e.target.closest('[data-filter]'),thumb=e.target.closest('[data-image]');if(product)openProduct(product.dataset.product,product);if(save)toggleSave(save.dataset.save);if(f){filter=f.dataset.filter;render()}if(thumb)setImage(thumb.dataset.image)});
...
document.querySelector('#dialog-save').onclick=()=>toggleSave(activeProduct.id);
document.querySelector('#dialog-inquire').onclick=()=>{productSelect.value=activeProduct.name;dialog.close();document.querySelector('#contact').scrollIntoView({behavior:'smooth'});setTimeout(()=>document.querySelector('input[name=name]').focus({preventScroll:true}),350)};
document.querySelector('.saved-button').onclick=()=>{filter='saved';render();document.querySelector('#collection').scrollIntoView()};
document.querySelector('#show-all').onclick=()=>{filter='all';render()};
document.querySelector('#explore-decor').onclick=()=>{filter='decor';render();document.querySelector('#collection').scrollIntoView()};
const menu=document.querySelector('.menu-toggle');
function closeMenu(){document.querySelector('nav').classList.remove('open');menu.setAttribute('aria-expanded','false');menu.setAttribute('aria-label','Open navigation');menu.innerHTML=icon('menu')}
menu.onclick=()=>{const open=document.querySelector('nav').classList.toggle('open');menu.setAttribute('aria-expanded',String(open));menu.setAttribute('aria-label',open?'Close navigation':'Open navigation');menu.innerHTML=icon(open?'close':'menu')};
